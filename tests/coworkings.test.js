...
describe('Coworkings API', () => {
  ...
  describe('GET /api/coworkings', () => {
    ...
    it('should filter by city', async () => {
      const response = await request(app)
        .get('/api/coworkings?city=Valencia')
        .expect(200);
    ...
    it('should filter by search term', async () => {
      const response = await request(app)
        .get('/api/coworkings?search=wayco')
        .expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.data.some(c => 
        c.name.toLowerCase().includes('wayco')
      )).toBe(true);
    });

    it('should filter by max price', async () => {
      const response = await request(app)
        .get('/api/coworkings?maxPrice=25')
        .expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.data.every(c => 
        c.pricing.dayPass <= 25
      )).toBe(true);
    });

    it('should filter by featured', async () => {
      const response = await request(app)
        .get('/api/coworkings?featured=true')
        .expect(200);
  ...
  describe('GET /api/coworkings/slug/:slug', () => {
    it('should return coworking by slug', async () => {
      const response = await request(app)
        .get('/api/coworkings/slug/wayco-valencia')
        .expect(200);
    ...
    it('should return 404 for non-existent slug', async () => {
      const response = await request(app)
        .get('/api/coworkings/slug/non-existent')
        .expect(404);
